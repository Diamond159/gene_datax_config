...
def insert_into_table_batch(rows):
    try:
        for row in rows:
            insert_query = """
    INSERT INTO TABLE yuqing.dwd_hotsearch_video_inc 
    ...
    VALUES (
		...
		""".format(operate_date=row["operate_date"],_id=row["_id"],brief_text=row["brief_text"],
			...
			video_urls=json.dumps(row["video_urls"])[1:-1],hot_point=row["hot_point"],author_name=row["author_name"],author_id=row["author_id"],root_url=row["root_url"] if row["root_url"] else "NULL",
